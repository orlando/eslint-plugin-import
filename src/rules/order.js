...
function mutateRanksToAlphabetize(imported, alphabetizeOptions) {
  ...
  const comparator = alphabetizeOptions.caseInsensitive
    ? (a, b) => sorterFn(String(a).toLowerCase(), String(b).toLowerCase())
    : (a, b) => sorterFn(a, b);
...
function mutateRanksToAesthetically(imported) {
  const lengths = imported
    .map(function (importedItem) {
      return importedItem.node.range[1] - importedItem.node.range[0];
    })
    .sort(function (a, b) {
      return b - a;
    });
  ...
  // mutate the original group-rank with alphabetized-rank
  imported.forEach(function (importedItem) {
    const range = importedItem.node.range[1] - importedItem.node.range[0];
    importedItem.rank = lengths.indexOf(range);
  });
}
...
module.exports = {
  ...
  create: function importOrderRule(context) {
    ...
    const aesthetically = options["aesthetically"] === true;
    let ranks;
    ...
    return {
      ...
      "Program:exit": function reportAndReset() {
        ...
        } else if (aesthetically) {
          mutateRanksToAesthetically(imported);
        }
